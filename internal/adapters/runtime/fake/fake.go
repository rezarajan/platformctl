...
// Replicas > 1 set (docs/design/004-replicas-and-identity.md). Keyed in
...
func (r *Runtime) ensureReplicaSetLocked(spec runtime.ContainerSpec, n int) runtime.ContainerState {
	...
	for i := 0; i < n; i++ {
		...
		if spec.StableIdentity {
			...
			// a StableIdentity set (docs/design/004) — a caller must not
...
// docs/design/004), and, when StableIdentity, ordinal-suffixed volume
...
// aggregate best-effort default documented in docs/design/004 ("Known
...
// documented best-effort default — docs/design/004 "Known limitations").
