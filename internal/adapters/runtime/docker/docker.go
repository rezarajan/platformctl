...
// returns the aggregate ContainerState (docs/adr/004): Healthy/Running
...
// EnsureVolume for it itself — docs/adr/004).
...
// volumes are deliberately left in place (docs/adr/004's conservative
...
// Healthy are true when at least one member is (docs/adr/004's "provider
...
func (r *Runtime) ensureOneContainer(ctx context.Context, spec runtime.ContainerSpec) (runtime.ContainerState, error) {
	...
	// headless Service (docs/adr/004-replicas-and-identity.md). spec.Name
...
// exists), resolves to ordinal 0 as a best-effort default (docs/adr/004,
...
func (r *Runtime) WaitHealthy(ctx context.Context, name string, timeout time.Duration) error {
	...
	for {
		...
		case errdefs.IsNotFound(err):
			...
			// "at least one member healthy" (docs/adr/004's "provider
...
// default (docs/adr/004, "Known limitations").
...
func (r *Runtime) Inspect(ctx context.Context, name string) (runtime.ContainerState, bool, error) {
	...
	// a replica set (docs/adr/004): Docker has no native object to name
