...
// s3sink (docs/planning/08 E6 done-note's recorded follow-up; ADR 028):
...
// Connect-worker family shape debezium/jdbcsink/s3source's own exemplars
...
// Connect REST client — JSON connector-status parsing (RUNNING/FAILED,
// task state), not a bare status-code check — with no dialer seam to
// intercept short of faking Connect's REST API surface, which ADR 028 §2's
// fake-honesty rule would require pinning against a real Connect worker's
// observed behavior before trusting a green result. Covered instead by the
// Docker integration suite (cmd/platformctl's sink/lakehouse scenarios).
//
		...
		CapabilityChecks: func(p reconciler.Provider) []conformance.CapabilityCheck {
			...
			return []conformance.CapabilityCheck{
				...
				},
				{
					...
					Invoke: func() error {
						return bov.ValidateBindingOptions("sink", map[string]any{"endpoint": "not-a-url"})
					},
