...
// s3source (docs/planning/08 E6 done-note's recorded follow-up; ADR 028):
...
// Kafka/Connect/S3 dial anywhere in the worker's own startup path), the
// same Connect-worker family shape debezium/s3sink/jdbcsink's own
...
// registers/diffs a live source connector via kafkaconnect's real Kafka
// Connect REST client — no dialer seam to intercept short of faking
// Connect's REST API surface, which ADR 028 §2's fake-honesty rule would
// require pinning against a real Connect worker's observed behavior before
// trusting a green result. Covered instead by the Docker integration suite
// (cmd/platformctl's ingest scenarios).
//
...
// exclusion) and BindingOptionsValidator (options.endpoint must be a
// well-formed URL).
	...
	conformance.Run(t, conformance.Harness{
		...
		CapabilityChecks: func(p reconciler.Provider) []conformance.CapabilityCheck {
			...
			return []conformance.CapabilityCheck{
				...
				{
					Name: "ValidateBindingOptions/endpoint-not-a-url",
					Invoke: func() error {
						return bov.ValidateBindingOptions("ingest", map[string]any{"endpoint": "not-a-url"})
					},
					WantSubstrings: []string{"options.endpoint", "is not a valid URL"},
				},
			}
		},
	})
