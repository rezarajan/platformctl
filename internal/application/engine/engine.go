			...
			if addr != "" {
				...
				if conn.External {
					if inerr := e.probeInNetworkUnreachable(ctx, env, addr, byKey); inerr != nil {
						st.SetCondition(status.Condition{Type: status.Ready, Status: status.False, Reason: "ExternalEndpointUnreachableInNetwork", Message: inerr.Error()}, now)
						st.SetCondition(status.Condition{Type: status.DriftDetected, Status: status.True, Reason: "ExternalEndpointUnreachableInNetwork"}, now)
						return st
					}
				}
