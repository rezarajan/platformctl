...
func TestDomainRuntimeQualifyTargetAddress(t *testing.T) {
	...
	// even across domains.
	dDocker := newDomainRuntime(fakeruntime.New(), map[string]any{}, caller, caller, nil, false, nil, "fake", nil).(*domainRuntime)
	...
	// namespace FQDN.
	dK8s := newDomainRuntime(fakeruntime.New(), map[string]any{}, caller, caller, nil, false, nil, "kubernetes", nil).(*domainRuntime)
	...
	dPinned := newDomainRuntime(fakeruntime.New(), map[string]any{"network": "custom-net"}, caller, caller, nil, false, nil, "kubernetes", nil).(*domainRuntime)
