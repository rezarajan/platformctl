...
docker-acceptance|internal/adapters/runtime/docker internal/adapters/providers/placeholder cmd/platformctl/testdata/docker-scenario SHARED_CORE|go test -tags integration -count=1 -run 'TestDockerProviderEndToEnd' -timeout 900s ./cmd/platformctl/
...
chaos-k8s|internal/adapters/runtime/kubernetes cmd/platformctl/testdata/chaos-k8s-scenario SHARED_CORE|go test -tags integration -count=1 -run 'TestKubernetesChaosApplyKilledMidRun' -timeout 1800s ./cmd/platformctl/
...
state-s3|internal/adapters/state internal/ports/state|go test -tags integration -count=1 -timeout 1200s ./internal/adapters/state/... ./cmd/platformctl/ -run 'TestSharedState|TestConformance|TestForceUnlock|TestLockReclaimsAfterExpiry'
