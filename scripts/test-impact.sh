...
suites() {
  cat <<'EOF'
docker-conformance|internal/adapters/runtime/docker internal/adapters/runtime/fake internal/ports/runtime internal/adapters/runtime/probe|go test -tags integration -count=1 -run 'Conformance|TestEnsureNetworkRefusesUnmanagedExisting|TestEnsureVolumeRefusesUnmanagedExisting|TestImagePullAuthPullsFromPrivateRegistry|TestNetworkAliasResolvesInNetwork|TestOutOfBandKillSurfacesUnhealthy|TestPublishedPortBindsToLoopbackByDefault|TestPullPolicyNeverFailsFastOnAbsentImage' -timeout 900s ./internal/adapters/runtime/docker/
