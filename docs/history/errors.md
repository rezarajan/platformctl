# Resolved error reports (archive)

> **Archived 2026-07-20** (moved from the repo root to `docs/history/`).
> Root-cause analyses of live failures, each resolved in the commits its
> entry names. New systemic lessons are folded into
> `docs/planning/09-systemic-findings-and-segregation-readiness.md` and the
> F6 conformance ratchet (doc 06 §8); this file is the raw incident record.
