...
| `spec.access` | array of object | no | Explicit wide-grant declarations (docs/adr/026 §2, docs/planning/08 H7) widening this Provider's reachability beyond its own declared graph edges to every resource in a named namespace. Only takes effect under the GraphScopedAccess feature gate (Alpha, disabled by default); schema-valid but inert otherwise. |
...
| `spec.runtime` | object | no | Where the provider's backing objects run. Fields beyond type are runtime-specific (e.g. network for docker). Optional (docs/adr/035 decision 1, docs/planning/08 M1): omitted, this Provider inherits the project's one runtime, declared once in datascape.yaml at the manifest path's root — the common, recommended path. Declared here, it is an explicit override, validated to match the project runtime's type or refused ('a project targets one runtime — put it in its own project folder'); with no datascape.yaml present at all, spec.runtime is required exactly as before (backward compat — see internal/domain/provider.Provider.validate). |
...
| `spec.runtime.resources` | object | no | Optional resource bounds applied to EVERY long-running container this Provider realizes (docs/planning/08 J5; docs/adr/029's sibling concern — unbounded workloads let JVMs size heaps from host memory and let the Kubernetes scheduler pack pods by count, the eviction-churn class CI exposed). Injected at the engine's runtime chokepoint, so providers need zero changes to honor it. kubernetes: limits/requests verbatim. docker: memory/cpu limits map to the daemon's cgroup bounds; cpuReservation is best-effort CPU shares. Short-lived job containers (backup/restore pipelines) are deliberately not bounded by this field. |
| `spec.runtime.resources.cpu` | number | no | CPU limit in cores (e.g. 0.5, 2). Unset = unlimited. |
| `spec.runtime.resources.cpuReservation` | number | no | CPU request/reservation in cores. kubernetes: scheduler-visible request. docker: best-effort CPU shares. |
| `spec.runtime.resources.memory` | string | no | Memory limit as an integer with a binary suffix (Ki|Mi|Gi), e.g. "512Mi", "1Gi". Container-aware JVMs size their default heap from this. |
| `spec.runtime.resources.memoryReservation` | string | no | Memory request/reservation, same format. kubernetes: the scheduler places by THIS value — setting it is what prevents overpacking. |
| `spec.runtime.type` | `docker` \| `fake` \| `kubernetes` \| `external` \| `terraform` | no | docker and fake (testing) are implemented. kubernetes is a real, Beta adapter behind the KubernetesRuntime feature gate (enabled by default as of docs/planning/08 Stage B close) — see runtime.access and the deploy/kubernetes/rbac/ manifests for what running against a real cluster needs. external/terraform are accepted for forward compatibility and rejected at registry construction as planned-but-unavailable. |
