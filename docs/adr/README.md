...
| [037](037-multi-runtime-materialization-intent-preservation.md) | Multi-runtime provider materialization: intent as a set, materializers as verified functions | proposed | Provider registers per-runtime materializers (Helm-backed k8s, containers on docker), selected by project runtime; Intent = invariant-core ⊎ best-effort-hints (size exact everywhere, tier degrades explicitly); HA→ordinals matched cross-runtime; translation formally verified (Alloy model + property-based + differential conformance) — the math does the heavy lifting |
