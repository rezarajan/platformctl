...
## v1.3.0 — feature-gate graduations (Alpha → Beta)

Fifteen gates graduate from Alpha to Beta **maturity** this release. This
is a quality signal, not a behavior change: **every gate keeps its
existing default** (all stay opt-in/disabled except `DesignLints`, which
stays enabled). Nothing you did not already opt into changes.

- No action required. A manifest that did not enable these gates behaves
  byte-for-byte as it did on v1.2.0.
- To adopt a now-Beta capability, enable its gate exactly as before, e.g.
  `--feature-gates TrinoProvider=true` or `--feature-gates
  MediatedConnections=true,PolicyEngine=true`.
- Graduated: HighAvailability, BackupRestore, MonitoringStackProvider,
  IngressProvider, TLSTermination, TrinoProvider, JDBCSinkProvider,
  IngestProvider, TunnelProvider, SharedStateBackend, MediatedConnections,
  PolicyEngine, GraphScopedAccess, LabelScopedAccess, DesignLints.
- Deliberately still Alpha (do not enable in production):
  `MediatedTransport` (the mediation-default-transport flip is incomplete),
  `ParallelReconciliation`, `VaultSecretBackend`.
