...
//
// (An earlier pack revision also shipped secrets-from-vault-or-k8s, a
// non-exemptible twin of the exemptible forbid-env-secret-backend on the
// same fact; the owner removed it — docs/adr/021 addendum, 2026-07-22 —
// the pack is a starter for local tailoring, and one rule per fact with
// a locally-flippable exemptible flag expresses the same governance
// without an unwaivable twin.)
//
// protect-data is recorded here deliberately rather than routed around:
// the pack is not weakened (no exemptible flag flip) and no exemption
// annotation is added (ADR 021 §3: a non-exemptible deny has no
// in-manifest escape, and applyExemptions ignores any annotation naming
// one anyway). This test's job is to catch *new* posture regressions —
// any unexempted deny whose rule id is not in this set fails the build.
